...
export default function FluidCursor() {
  ...
  useEffect(() => {
    ...
    let config = {
      ...
      DENSITY_DISSIPATION: 3.5, // Reduced dissipation for longer trails
      VELOCITY_DISSIPATION: 2,
      PRESSURE: 0.1,
      PRESSURE_ITERATIONS: 20,
      CURL: 3,
      SPLAT_RADIUS: 0.12, // Reduced from 0.2 for smaller size
      SPLAT_FORCE: 60000,
      SHADING: true,
    ...
    const context = getWebGLContext(canvas) as { gl: any; ext: any };
    const gl = context.gl;
  ...
  `,
    ...
    function generateColor() {
      const pink = { r: 0.988 * 0.5, g: 0.274 * 0.5, b: 0.419 * 0.5 };
      const blue = { r: 0.247 * 0.5, g: 0.368 * 0.5, b: 0.984 * 0.5 };
      const gray = 0.5;
      const mix = (c: number, g: number, f: number) => c * f + g * (1 - f);
      const muteFactor = 0.7; // 70% original, 30% gray
      const t = Math.random();
      return {
        r: mix(pink.r + t * (blue.r - pink.r), gray * 0.2, muteFactor),
        g: mix(pink.g + t * (blue.g - pink.g), gray * 0.2, muteFactor),
        b: mix(pink.b + t * (blue.b - pink.b), gray * 0.2, muteFactor)
      };
    }
    ...
    // Export toggle behaviors globally or wire them directly in ClientInitializer
    const fluidInstance = {
      stop: () => {
        isRunning = false;
        if (animationFrameId) cancelAnimationFrame(animationFrameId);
